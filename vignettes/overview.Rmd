...
Another neat thing we can try to do with the `scores()` function is to rank the teams that won on a given day by how many goals they scored. We'll use the `dplyr` package to help out.
...
scores %>%
	mutate(winning = home_score - away_score) %>%
	mutate(winner <- if(winning > 0) "home wins" else "away wins")
```
